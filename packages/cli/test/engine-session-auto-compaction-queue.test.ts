...
import { type AssistantMessage, getModel } from "astro-core";
...
describe("EngineSession auto-compaction queue resume", () => {
	let session: EngineSession;
	let sessionManager: SessionManager;
	let tempDir: string;

	beforeEach(() => {
		tempDir = join(tmpdir(), `astroagent-auto-compaction-queue-${Date.now()}`);
	...
	it("should resume after threshold compaction when only engine-level queued messages exist", async () => {
		session.engine.followUp({
			role: "custom",
			customType: "test",
		...
		expect(session.engine.hasQueuedMessages()).toBe(true);

		const continueSpy = vi.spyOn(session.engine, "continue").mockResolvedValue();
	...
	it("should trigger threshold compaction for error messages using last successful usage", async () => {
		...
		// Put both messages into engine state so estimateContextTokens can find the successful one
	...
	it("should not trigger threshold compaction for error messages when only kept pre-compaction usage exists", async () => {
		...
		// Engine state has the kept assistant (pre-compaction) and the error (post-compaction)
		session.engine.state.messages = [
