...
describe("edit tool fuzzy matching", () => {
	...
	beforeEach(() => {
		testDir = join(tmpdir(), `cli-fuzzy-test-${Date.now()}`);
		mkdirSync(testDir, { recursive: true });
	});
	...
	it("should match fullwidth punctuation in Chinese text", async () => {
		...
		const result = await editTool.execute("test-fuzzy-chinese", {
			path: testFile,
			edits: [{ oldText: "你好,世界\n你好(世界)\n", newText: "你好，AstroAgent\n你好(AstroAgent)\n" }],
		});

		expect(getTextOutput(result)).toContain("Successfully replaced");
		const content = readFileSync(testFile, "utf-8");
		expect(content).toBe("你好，AstroAgent\n你好(AstroAgent)\n");
	});
...
describe("edit tool CRLF handling", () => {
	...
	beforeEach(() => {
		testDir = join(tmpdir(), `cli-crlf-test-${Date.now()}`);
		mkdirSync(testDir, { recursive: true });
	});
