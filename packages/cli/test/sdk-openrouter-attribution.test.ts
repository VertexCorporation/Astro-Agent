...
describe("createEngineSession OpenRouter attribution headers", () => {
	let tempDir: string;
	let cwd: string;
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `astroagent-sdk-openrouter-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	async function captureHeaders(
		...
		const modelRegistry = ModelRegistry.create(authStorage, join(engineDir, "models.json"));
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://AstroAgent.dev");
		expect(headers?.["X-OpenRouter-Title"]).toBe("AstroAgent");
