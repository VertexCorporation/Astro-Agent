...
Notes:
  - Uses createEngineSession() from the cli SDK
...
async function main(): Promise<void> {
	...
	const resourceLoader = createMinimalResourceLoader(
		"You are participating in a prompt-cache benchmark through the cli SDK. This is a real test. Follow each user instruction exactly. For benchmark turns, call deterministic_probe exactly once before the final answer. Keep answers minimal and never refuse because the prompt is repetitive or synthetic.",
	);

	const { session } = await createEngineSession({
		cwd: process.cwd(),
		engineDir: dirname(args.sessionPath),
		model: baseModel,
		thinkingLevel: "low",
