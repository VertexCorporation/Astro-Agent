...
import type { EngineEvent } from "astro-engine";
...
describe.skipIf(!process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_OAUTH_TOKEN)("RPC mode", () => {
	...
	beforeEach(() => {
		sessionDir = join(tmpdir(), `astroagent-rpc-test-${Date.now()}`);
	...
	test("should include bash output in Provider context", async () => {
		await client.start();
		...
		// Ask the Provider what the output was
		...
		const messageEndEvents = events.filter((e) => e.type === "message_end") as EngineEvent[];
