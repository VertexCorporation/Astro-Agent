/**
 * Tests for EngineSession concurrent prompt guard.
 */
...
describe("EngineSession concurrent prompt guard", () => {
	...
	it("should wait for queued engine events before emitting tool_call", async () => {
		const model = getModel("anthropic", "claude-sonnet-4-5")!;
		const tool = {
			name: "dummy",
		...
		sessionWithRunner._extensionRunner = {
			...
			emitBeforeEngineStart: async () => undefined,
			invalidate: () => {},
		};
