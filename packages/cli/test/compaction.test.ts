...
function createUserMessage(text: string): EngineMessage {
...
function createMessageEntry(message: EngineMessage): SessionMessageEntry {
...
function extractText(messages: EngineMessage[]): string {
	return messages
		.map((message) => {
			switch (message.role) {
				case "user":
...
// ============================================================================
// Provider integration tests (skipped without API key)
// ============================================================================

describe.skipIf(!process.env.ANTHROPIC_OAUTH_TOKEN)("Provider summarization", () => {
