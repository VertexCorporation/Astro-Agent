/**
 * Test harness for EngineSession runtime testing.
 *
 * Provides:
 * - A faux stream function with declarative response sequencing
 * - A one-call factory for a fully wired EngineSession with real in-memory dependencies
...
export interface HarnessOptions {
	...
	systemPrompt?: string;
	/** Custom tools to register on the engine. */
	tools?: EngineTool[];
...
function createHarnessWithResourceLoader(
	...
	return {
		...
		eventsOfType<T extends EngineSessionEvent["type"]>(type: T) {
			return events.filter((e): e is Extract<EngineSessionEvent, { type: T }> => e.type === type);
		},
		tempDir,
		cleanup,
	};
