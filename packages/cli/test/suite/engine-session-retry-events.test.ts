import { fauxAssistantMessage, fauxThinking, fauxToolCall } from "astro-core";
...
describe("EngineSession retry and event characterization", () => {
	...
	it("prompt waits for retry completion even when assistant message_end handling is delayed", async () => {
		const harness = await createHarness({
			...
			extensionFactories: [
				(AstroAgent) => {
					AstroAgent.on("message_end", async (event) => {
	...
	it("emits extension events before public event subscribers", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(AstroAgent) => {
					AstroAgent.on("message_start", async (event) => {
	...
	it("emits engine_end for error responses", async () => {
		const harness = await createHarness();
		harnesses.push(harness);
		...
		expect(harness.events[harness.events.length - 1]?.type).toBe("engine_end");
	});

	it("emits engine_end for aborted runs and persists the aborted assistant message", async () => {
		const harness = await createHarness();
		harnesses.push(harness);
