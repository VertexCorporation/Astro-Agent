...
describe("EngineSession prompt characterization", () => {
	...
	it("executes multiple tool calls from one response and continues with a single follow-up response", async () => {
		const toolRuns: string[] = [];
		const makeTool = (name: string, delayMs: number): EngineTool => ({
			name,
			label: name,
	...
	it("expands skill commands before sending the prompt", async () => {
		const tempDir = join(tmpdir(), `astroagent-skill-${Date.now()}-${Math.random().toString(36).slice(2)}`);
