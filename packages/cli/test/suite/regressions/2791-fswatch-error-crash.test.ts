...
/**
 * Regression test for https://github.com/badlogic/Astro-Agent/issues/2791
 *
...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "astroagent-2791-"));
	...
	it("process should survive an error event on the theme FSWatcher", () => {
		...
		const engineDir = join(tempRoot, "engine").replace(/\\/g, "/");
...
`,
		...
		try {
			_stdout = execFileSync("npx", ["tsx", scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, PI_CODING_AGENT_DIR: engineDir },
