/**
 * Local test harness for the new cli test suite.
 */

import { existsSync, mkdirSync, rmSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "astro-core";
import { registerFauxProvider } from "astro-core";
import type { EngineMessage, EngineTool } from "astro-engine";
...
export interface Harness {
	...
	eventsOfType<T extends EngineSessionEvent["type"]>(type: T): Extract<EngineSessionEvent, { type: T }>[];
	tempDir: string;
	cleanup: () => void;
}

function createTempDir(): string {
	const tempDir = join(tmpdir(), `astroagent-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
export async function createHarness(options: HarnessOptions = {}): Promise<Harness> {
	...
	const engine = new Engine({
		...
		transformContext: async (messages: EngineMessage[]) => {
			const runner = extensionRunnerRef.current;
	...
	return {
		...
		events,
		eventsOfType<T extends EngineSessionEvent["type"]>(type: T) {
			return events.filter((event): event is Extract<EngineSessionEvent, { type: T }> => event.type === type);
		},
		tempDir,
		cleanup() {
			session.dispose();
