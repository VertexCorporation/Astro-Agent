import { type AssistantMessage, fauxAssistantMessage, type Model } from "astro-core";
...
describe("EngineSession compaction characterization", () => {
	...
	it("manually compacts using an extension-provided summary", async () => {
		const harness = await createHarness({
			extensionFactories: [
				(AstroAgent) => {
					AstroAgent.on("session_before_compact", async (event) => ({
						compaction: {
	...
	it("resumes after threshold compaction when only engine-level queued messages exist", async () => {
		vi.useFakeTimers();
		const harness = await createHarness({
		...
		harness.session.engine.followUp({
			role: "custom",
			customType: "test",
		...
		const continueSpy = vi.spyOn(harness.session.engine, "continue").mockResolvedValue();
	...
	it("triggers threshold compaction for error messages using the last successful usage", async () => {
		...
		harness.session.engine.state.messages = [
