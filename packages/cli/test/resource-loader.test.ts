...
describe("DefaultResourceLoader", () => {
	...
	describe("reload", () => {
		...
		it("should discover skills from engineDir", async () => {
...
Skill content here.`,
		...
		it("should discover prompts from engineDir", async () => {
...
Project skill`,
	...
	AstroAgent.registerCommand("user-only", {
		...
		it("should honor overrides for auto-discovered resources", async () => {
			...
			const extensionsDir = join(engineDir, "extensions");
			mkdirSync(extensionsDir, { recursive: true });
			writeFileSync(join(extensionsDir, "disabled.ts"), "export default function() {}");

			const skillDir = join(engineDir, "skills", "skip-skill");
			mkdirSync(skillDir, { recursive: true });
			writeFileSync(
				join(skillDir, "SKILL.md"),
				`---
...
Content`,
			...
			const loader = new DefaultResourceLoader({ cwd, engineDir, settingsManager });
			await loader.reload();
		...
		it("should discover AGENTS.md context files", async () => {
			...
			const { enginesFiles } = loader.getEnginesFiles();
			expect(enginesFiles.some((f) => f.path.includes("AGENTS.md"))).toBe(true);
		});
		...
		it("should skip AGENTS.md and CLAUDE.md discovery when noContextFiles is true", async () => {
			...
			const loader = new DefaultResourceLoader({ cwd, engineDir, noContextFiles: true });
			await loader.reload();
...
Content`,
			...
			const loader = new DefaultResourceLoader({ cwd, engineDir, noSkills: true });
			await loader.reload();
...
Content`,
	...
	describe("extension conflict detection", () => {
		it("should detect tool conflicts between extensions", async () => {
			...
			const ext1Dir = join(engineDir, "extensions", "ext1");
			const ext2Dir = join(engineDir, "extensions", "ext2");
  ...
  AstroAgent.registerTool({
    ...
    execute: async () => ({ result: "2" }),
		...
		it("should prefer explicit CLI extensions over discovered extensions when commands and tools conflict", async () => {
			const globalExtDir = join(engineDir, "extensions");
