...
describe("test harness", () => {
	...
	it("event capture", async () => {
		...
		const engineStarts = harness.eventsOfType("engine_start");
		expect(engineStarts).toHaveLength(1);

		const engineEnds = harness.eventsOfType("engine_end");
		expect(engineEnds).toHaveLength(1);
