...
describe("package commands", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `astroagent-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("uses global npmCommand for self updates", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@mariozechner", "astroagent-cli");
...
`,
		...
		writeFileSync(
			join(projectDir, ".astroagent", "settings.json"),
			JSON.stringify({ npmCommand: [originalExecPath, fakeNpmPath, "--prefix", projectPrefix] }, null, 2),
		);
		process.env.ASTRO_PACKAGE_DIR = selfPackageDir;
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(engineDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:astroagent-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "astroagent-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:astroagent-formatter?");
			expect(stdout).not.toContain("Updated astroagent-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:astroagent-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
