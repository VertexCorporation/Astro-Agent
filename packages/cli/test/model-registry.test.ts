...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "astro-core";
import { getApiProvider } from "astro-core";
import { getOAuthProvider } from "astro-core/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `astroagent-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
