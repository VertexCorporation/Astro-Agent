...
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "astro-core";
import { Engine, type EngineEvent, type EngineTool } from "astro-engine";
...
type SessionWithExtensionEmitHook = {
	_emitExtensionEvent: (event: EngineEvent) => Promise<void>;
};

describe("EngineSession retry", () => {
	let session: EngineSession;
	let tempDir: string;

	beforeEach(() => {
		tempDir = join(tmpdir(), `astroagent-retry-test-${Date.now()}`);
	...
	function createSession(options?: { failCount?: number; maxRetries?: number; delayAssistantMessageEndMs?: number }) {
		...
		if (delayAssistantMessageEndMs > 0) {
			...
			sessionWithHook._emitExtensionEvent = async (event: EngineEvent) => {
	...
	it("prompt waits for full engine loop when retry produces tool calls", async () => {
		...
		// waitForRetry() while the engine was still executing tools.
		let callCount = 0;
		...
		// All three Provider calls must have completed
		expect(callCount).toBe(3);
		// Tool must have been executed
		expect(toolExecuted.value).toBe(true);
		// Engine must not be streaming after prompt returns
		expect(session.isStreaming).toBe(false);
		// A follow-up prompt must work (no "Engine is already processing" error)
