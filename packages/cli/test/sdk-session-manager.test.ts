...
describe("createEngineSession session manager defaults", () => {
	let tempDir: string;
	let cwd: string;
	let engineDir: string;

	beforeEach(() => {
		tempDir = join(tmpdir(), `astroagent-sdk-session-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("uses engineDir for the default persisted session path", async () => {
		...
		const expectedSessionDir = join(engineDir, "sessions", safePath);
	...
	it("derives cwd from an explicit sessionManager when cwd is omitted", async () => {
		...
		const bashTool = session.engine.state.tools.find((tool) => tool.name === "bash");
