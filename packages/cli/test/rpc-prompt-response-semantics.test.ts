...
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel, type Model } from "astro-core";
...
function createRuntimeHost(options: { withAuth: boolean; responseDelayMs: number; model?: Model<any> }): {
	...
	const tempDir = join(tmpdir(), `astroagent-rpc-prompt-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	} as unknown as EngineSessionRuntime;
