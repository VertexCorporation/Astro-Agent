...
describe("DefaultPackageManager", () => {
	...
	describe("resolve", () => {
		...
		it("should resolve local extension paths from settings", async () => {
			const extDir = join(engineDir, "extensions");
		...
		it("should resolve skill paths from settings", async () => {
			const skillDir = join(engineDir, "skills", "my-skill");
...
Content`,
		...
		it("should auto-discover root markdown skills from .astroagent skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .astroagent", async () => {
			const extDir = join(tempDir, ".astroagent", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".astroagent"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".astroagent", "extensions"), directoryLinkType);
				symlinkSync(sharedSkillsDir, join(tempDir, ".astroagent", "skills"), directoryLinkType);
				symlinkSync(sharedPromptsDir, join(tempDir, ".astroagent", "prompts"), directoryLinkType);
				symlinkSync(sharedThemesDir, join(tempDir, ".astroagent", "themes"), directoryLinkType);
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".astroagent", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json AstroAgent.extensions in extensions setting", async () => {
			// Create a package with AstroAgent.extensions in package.json
			...
			// Should find the extensions declared in package.json AstroAgent.extensions
	...
	describe(".engines/skills auto-discovery", () => {
		it("should scan .engines/skills from cwd up to git repo root", async () => {
			const repoRoot = join(tempDir, "repo");
			const nestedCwd = join(repoRoot, "packages", "feature");
			mkdirSync(nestedCwd, { recursive: true });
			mkdirSync(join(repoRoot, ".git"), { recursive: true });

			const aboveRepoSkill = join(tempDir, ".engines", "skills", "above-repo", "SKILL.md");
			mkdirSync(join(tempDir, ".engines", "skills", "above-repo"), { recursive: true });
			writeFileSync(aboveRepoSkill, "---\nname: above-repo\ndescription: above\n---\n");

			const repoRootSkill = join(repoRoot, ".engines", "skills", "repo-root", "SKILL.md");
			mkdirSync(join(repoRoot, ".engines", "skills", "repo-root"), { recursive: true });
			writeFileSync(repoRootSkill, "---\nname: repo-root\ndescription: repo\n---\n");

			const nestedSkill = join(repoRoot, "packages", ".engines", "skills", "nested", "SKILL.md");
			mkdirSync(join(repoRoot, "packages", ".engines", "skills", "nested"), { recursive: true });
		...
		it("should scan .engines/skills up to filesystem root when not in a git repo", async () => {
			const nonRepoRoot = join(tempDir, "non-repo");
			const nestedCwd = join(nonRepoRoot, "a", "b");
			mkdirSync(nestedCwd, { recursive: true });

			const rootSkill = join(nonRepoRoot, ".engines", "skills", "root", "SKILL.md");
			mkdirSync(join(nonRepoRoot, ".engines", "skills", "root"), { recursive: true });
			writeFileSync(rootSkill, "---\nname: root\ndescription: root\n---\n");

			const middleSkill = join(nonRepoRoot, "a", ".engines", "skills", "middle", "SKILL.md");
			mkdirSync(join(nonRepoRoot, "a", ".engines", "skills", "middle"), { recursive: true });
		...
		it("should ignore root markdown files in .engines/skills", async () => {
			const enginesSkillsDir = join(tempDir, ".engines", "skills");
			mkdirSync(join(enginesSkillsDir, "nested-skill"), { recursive: true });
			const rootSkill = join(enginesSkillsDir, "root-file.md");
			const nestedSkill = join(enginesSkillsDir, "nested-skill", "SKILL.md");
		...
		it("should keep ~/.engines/skills user-scoped when cwd is under home in a non-git directory", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;

			try {
				const cwd = join(tempDir, "scratch", "nested");
				const localEngineDir = join(tempDir, ".astroagent", "engine");
				const localSettingsManager = SettingsManager.inMemory();
				mkdirSync(cwd, { recursive: true });
				mkdirSync(localEngineDir, { recursive: true });

				const homeSkill = join(tempDir, ".engines", "skills", "home-skill", "SKILL.md");
				mkdirSync(join(tempDir, ".engines", "skills", "home-skill"), { recursive: true });
				writeFileSync(homeSkill, "---\nname: home-skill\ndescription: home\n---\n");

				const pm = new DefaultPackageManager({
					cwd,
					engineDir: localEngineDir,
		...
		it("should dedupe user skill entries when ~/.astroagent/engine/skills is a symlink to ~/.engines/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;

			try {
				const engineSkillsDir = join(engineDir, "skills");
				const enginesSkillsDir = join(tempDir, ".engines", "skills");
				mkdirSync(enginesSkillsDir, { recursive: true });
				// Use junction on Windows to avoid EPERM when symlink privileges are unavailable.
				const directoryLinkType = process.platform === "win32" ? "junction" : "dir";
				symlinkSync(enginesSkillsDir, engineSkillsDir, directoryLinkType);

				const skillPath = join(enginesSkillsDir, "foo", "SKILL.md");
				mkdirSync(join(enginesSkillsDir, "foo"), { recursive: true });
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .astroagent auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".astroagent\n");

			const skillDir = join(tempDir, ".astroagent", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with AstroAgent manifest", async () => {
	...
	describe("npmCommand", () => {
		...
		it("should install git package dependencies with --omit=dev", async () => {
			...
			const targetDir = join(engineDir, "git", "github.com", "user", "repo");
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".astroagent", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("source parsing", () => {
		...
		it("should never parse dot-relative paths as git", () => {
			const dotSlash = (packageManager as any).parseSource("./packages/engine-timers");
			expect(dotSlash.type).toBe("local");
			expect(dotSlash.path).toBe("./packages/engine-timers");

			const dotDotSlash = (packageManager as any).parseSource("../packages/engine-timers");
			expect(dotDotSlash.type).toBe("local");
			expect(dotDotSlash.path).toBe("../packages/engine-timers");
		});
	});

	describe("settings source normalization", () => {
		it("should store global local packages relative to engine settings base", () => {
			...
			const rel = relative(engineDir, pkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});

		it("should store project local packages relative to .astroagent settings base", () => {
			...
			const rel = relative(join(tempDir, ".astroagent"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in top-level arrays", () => {
		...
		it("should filter themes with glob patterns", async () => {
			const themesDir = join(engineDir, "themes");
	...
	describe("pattern filtering in AstroAgent manifest", () => {
	...
	describe("multi-file extension discovery (issue #1102)", () => {
		it("should only load index.ts from subdirectories, not helper modules", async () => {
			...
			// should only load the index.ts entry point, not helper modules like engines.ts
			const pkgDir = join(tempDir, "multifile-pkg");
			mkdirSync(join(pkgDir, "extensions", "subengine"), { recursive: true });

			// Main entry point
			writeFileSync(
				join(pkgDir, "extensions", "subengine", "index.ts"),
				`import { helper } from "./engines.js";
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
			...
			writeFileSync(
				join(pkgDir, "extensions", "subengine", "engines.ts"),
			...
			expect(result.extensions.some((r) => pathEndsWith(r.path, "subengine/index.ts") && r.enabled)).toBe(true);
			expect(result.extensions.some((r) => pathEndsWith(r.path, "standalone.ts") && r.enabled)).toBe(true);

			// Should NOT find engines.ts as a standalone extension
			expect(result.extensions.some((r) => pathEndsWith(r.path, "engines.ts"))).toBe(false);
		});

		it("should respect package.json AstroAgent.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".astroagent", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".astroagent", "npm")],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned and current packages", async () => {
			vi.spyOn(packageManager as any, "getGlobalNpmRoot").mockReturnValue(join(engineDir, "node_modules"));
			...
			const projectOldPath = join(tempDir, ".astroagent", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".astroagent", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".astroagent", "npm", "node_modules", "example");
