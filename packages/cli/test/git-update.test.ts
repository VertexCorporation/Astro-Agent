...
describe("DefaultPackageManager git update", () => {
	...
	let engineDir: string; // The engine directory where extensions are installed
	...
	beforeEach(() => {
		...
		// This matches the path structure: engineDir/git/<host>/<path>
		installedDir = join(engineDir, "git", "github.com", "test", "extension");
	...
	function setupRemoteAndInstall(sourceOverride?: string): void {
		...
		mkdirSync(join(engineDir, "git", "github.com", "test"), { recursive: true });
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const cachedDir = join(tmpdir(), "astroagent-extensions", `git-${gitHost}`, hash, gitPath);
			const extensionFile = join(cachedDir, "astroagent-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "astroagent-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ AstroAgent: { extensions: ["./astroagent-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "astroagent-extensions/session-breakdown.ts")).toBe("// fresh");
		});
		...
		it("should not refresh pinned temporary git sources", async () => {
			...
			expect(getFileContent(cachedDir, "astroagent-extensions/session-breakdown.ts")).toBe("// pinned");
		});
	});
	...
	describe("scope-aware update", () => {
		it("should not install locally when source is only registered globally", async () => {
			...
			const projectGitDir = join(tempDir, ".astroagent", "git", "github.com", "test", "extension");
