/**
 * E2E tests for EngineSession compaction behavior.
 *
 * These tests use real Provider calls (no mocking) to verify:
...
describe.skipIf(!API_KEY)("EngineSession compaction e2e", () => {
	let session: EngineSession;
	let tempDir: string;
	let sessionManager: SessionManager;
	let events: EngineSessionEvent[];

	beforeEach(() => {
		// Create temp directory for session files
		tempDir = join(tmpdir(), `astroagent-compaction-test-${Date.now()}`);
	...
	it("should maintain valid session state after compaction", async () => {
		...
		// The engine should have responded
