import { type AssistantMessage, getModel, type Usage } from "astro-core";
...
function syncEngineMessages(session: EngineSession, sessionManager: SessionManager): void {
	session.engine.state.messages = sessionManager.buildSessionContext().messages;
}

describe("EngineSession.getSessionStats", () => {
	it("exposes the current context usage alongside token totals", () => {
		...
		try {
			...
			syncEngineMessages(session, sessionManager);
