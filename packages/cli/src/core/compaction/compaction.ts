...
/**
 * Extract EngineMessage from an entry if it produces one.
 * Returns undefined for entries that don't contribute to Provider context.
 */
function getMessageFromEntry(entry: SessionEntry): EngineMessage | undefined {
...
function getMessageFromEntryForCompaction(entry: SessionEntry): EngineMessage | undefined {
	if (entry.type === "compaction") {
		return undefined;
	}
 ...
 */
function getAssistantUsage(msg: EngineMessage): Usage | undefined {
...
function getLastAssistantUsageInfo(messages: EngineMessage[]): { usage: Usage; index: number } | undefined {
 ...
 */
export function estimateContextTokens(messages: EngineMessage[]): ContextUsageEstimate {
...
/**
 * Estimate token count for a message using chars/4.5 heuristic.
 * More accurate than chars/3.5 — matches real tokenizer ratios better.
 */
export function estimateTokens(message: EngineMessage): number {
	let chars = 0;
...
const SUMMARIZATION_PROMPT = `The messages above are a conversation to summarize. Create a structured context checkpoint summary that another Provider will use to continue the work.
...
export async function generateSummary(
	...
	const maxTokens = Math.min(8192, Math.floor(0.6 * reserveTokens));
	...
	// Convert to Provider messages first (handles custom types like bashExecution, custom, etc.)
...
export interface CompactionPreparation {
	...
	messagesToSummarize: EngineMessage[];
	/** Messages that will be turned into turn prefix summary (if splitting) */
	turnPrefixMessages: EngineMessage[];
...
export function prepareCompaction(
	...
	const messagesToSummarize: EngineMessage[] = [];
	...
	const turnPrefixMessages: EngineMessage[] = [];
