...
export interface ResourceLoader {
	...
	getEnginesFiles(): { enginesFiles: Array<{ path: string; content: string }> };
...
export function loadProjectContextFiles(options: {
	...
	const resolvedEngineDir = options.engineDir;

	const contextFiles: Array<{ path: string; content: string }> = [];
	const seenPaths = new Set<string>();

	const globalContext = loadContextFileFromDir(resolvedEngineDir);
	if (globalContext) {
...
export interface DefaultResourceLoaderOptions {
	...
	enginesFilesOverride?: (base: { enginesFiles: Array<{ path: string; content: string }> }) => {
		enginesFiles: Array<{ path: string; content: string }>;
	};
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	private enginesFilesOverride?: (base: { enginesFiles: Array<{ path: string; content: string }> }) => {
		enginesFiles: Array<{ path: string; content: string }>;
	};
	...
	private enginesFiles: Array<{ path: string; content: string }>;
	...
	constructor(options: DefaultResourceLoaderOptions) {
		...
		this.settingsManager = options.settingsManager ?? SettingsManager.create(this.cwd, this.engineDir);
		...
		this.enginesFilesOverride = options.enginesFilesOverride;
	...
	getEnginesFiles(): { enginesFiles: Array<{ path: string; content: string }> } {
		return { enginesFiles: this.enginesFiles };
	}
	...
	async reload(): Promise<void> {
		...
		const enginesFiles = {
			enginesFiles: this.noContextFiles ? [] : loadProjectContextFiles({ cwd: this.cwd, engineDir: this.engineDir }),
		};
		const resolvedEnginesFiles = this.enginesFilesOverride ? this.enginesFilesOverride(enginesFiles) : enginesFiles;
		this.enginesFiles = resolvedEnginesFiles.enginesFiles;
	...
	private getDefaultSourceInfoForPath(filePath: string): SourceInfo {
		...
		const engineRoots = [
			join(this.engineDir, "skills"),
			join(this.engineDir, "prompts"),
			join(this.engineDir, "themes"),
			join(this.engineDir, "extensions"),
		];
		const projectRoots = [
	...
	private loadThemes(
		...
		if (includeDefaults) {
			const defaultDirs = [join(this.engineDir, "themes"), join(this.cwd, CONFIG_DIR_NAME, "themes")];
	...
	private discoverSystemPromptFile(): string | undefined {
		...
		const globalPath = join(this.engineDir, "SYSTEM.md");
	...
	private discoverAppendSystemPromptFile(): string | undefined {
		...
		const globalPath = join(this.engineDir, "APPEND_SYSTEM.md");
		if (existsSync(globalPath)) {
			return globalPath;
		}
