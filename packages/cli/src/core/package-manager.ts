...
function collectAncestorEnginesSkillDirs(startDir: string): string[] {
	...
	while (true) {
		skillDirs.push(join(dir, ".engines", "skills"));
...
export class DefaultPackageManager implements PackageManager {
	...
	async resolve(onMissing?: (source: string) => Promise<MissingSourceAction>): Promise<ResolvedPaths> {
		...
		const globalBaseDir = this.engineDir;
	...
	private getGitInstallPath(source: GitSource, scope: SourceScope): string {
		...
		return join(this.engineDir, "git", source.host, source.path);
	}
	...
	private getGitInstallRoot(scope: SourceScope): string | undefined {
		...
		return join(this.engineDir, "git");
	}
	...
	private addAutoDiscoveredResources(
		...
		const userEnginesSkillsDir = join(getHomeDir(), ".engines", "skills");
		const projectEnginesSkillDirs = collectAncestorEnginesSkillDirs(this.cwd).filter(
			(dir) => resolve(dir) !== resolve(userEnginesSkillsDir),
		);
