...
import type { ImageContent, Model } from "astro-core";
...
/** Combined result from all before_engine_start handlers */
interface BeforeEngineStartCombinedResult {
	messages?: NonNullable<BeforeEngineStartEventResult["message"]>[];
	systemPrompt?: string;
}
...
export class ExtensionRunner {
	...
	async emitMessageEnd(event: MessageEndEvent): Promise<EngineMessage | undefined> {
		const ctx = this.createContext();
	...
	async emitContext(messages: EngineMessage[]): Promise<EngineMessage[]> {
		const ctx = this.createContext();
	...
	async emitBeforeEngineStart(
		prompt: string,
		images: ImageContent[] | undefined,
		systemPrompt: string,
		systemPromptOptions: BuildSystemPromptOptions,
	): Promise<BeforeEngineStartCombinedResult | undefined> {
		...
		const messages: NonNullable<BeforeEngineStartEventResult["message"]>[] = [];
		let systemPromptModified = false;

		for (const ext of this.extensions) {
			const handlers = ext.handlers.get("before_engine_start");
			if (!handlers || handlers.length === 0) continue;

			for (const handler of handlers) {
				try {
					const event: BeforeEngineStartEvent = {
					...
					if (handlerResult) {
						const result = handlerResult as BeforeEngineStartEventResult;
						if (result.message) {
