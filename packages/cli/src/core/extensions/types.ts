 ...
 * - Register Provider-callable tools
...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `Astro-Agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "Astro-Agent";
	 *
...
export interface ContextUsage {
	/** Estimated context tokens, or null if unknown (e.g. right after compaction, before next Provider response). */
	tokens: number | null;
...
export interface ExtensionContext {
	...
	/** Whether the engine is idle (not streaming) */
	isIdle(): boolean;
	/** The current abort signal, or undefined when the engine is not streaming. */
	signal: AbortSignal | undefined;
	/** Abort the current engine operation */
	abort(): void;
	/** Whether there are queued messages waiting */
	hasPendingMessages(): boolean;
	/** Gracefully shutdown Astro-Agent and exit. Available in all contexts. */
	shutdown(): void;
...
export interface ExtensionCommandContext extends ExtensionContext {
	/** Wait for the engine to finish streaming */
...
export interface ToolDefinition<TParams extends TSchema = TSchema, TDetails = unknown, TState = any> {
	...
	execute(
		...
		signal: AbortSignal | undefined,
		onUpdate: EngineToolUpdateCallback<TDetails> | undefined,
	...
	renderResult?: (
		result: EngineToolResult<TDetails>,
...
/** Fired before each Provider call. Can modify messages. */
...
/** Fired after user submits prompt but before engine loop. */
export interface BeforeEngineStartEvent {
	type: "before_engine_start";
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what Astro-Agent loaded without re-discovering resources. */
	systemPromptOptions: BuildSystemPromptOptions;
}

/** Fired when an engine loop starts */
export interface EngineStartEvent {
	type: "engine_start";
}

/** Fired when an engine loop ends */
export interface EngineEndEvent {
	type: "engine_end";
	messages: EngineMessage[];
}
...
export interface UserBashEvent {
	...
	command: string;
	/** True if !! prefix was used (excluded from Provider context) */
...
/** Fired when user input is received, before engine processing */
...
export interface BeforeEngineStartEventResult {
...
export interface ExtensionAPI {
	...
	/** Register a tool that the Provider can call. */
	...
	/** Append a custom entry to the session for state persistence (not sent to Provider). */
	 ...
	 * Astro-Agent.registerProvider("my-proxy", {
	 ...
	 * Astro-Agent.registerProvider("anthropic", {
	 *   baseUrl: "https://proxy.example.com"
	 * });
	 *
	 * @example
	 * // Register provider with OAuth support
	 * Astro-Agent.registerProvider("corporate-ai", {
	 *   baseUrl: "https://ai.corp.com",
	 *   api: "openai-responses",
	 *   models: [...],
	 *   oauth: {
	 *     name: "Corporate Core (SSO)",
	 ...
	 *
	 * @example
	 * Astro-Agent.unregisterProvider("my-proxy");
	 */
...
/** Configuration for registering a provider via Astro-Agent.registerProvider(). */
...
export interface ProviderModelConfig {
	...
	reasoning: boolean;
	/** Maps Astro-Agent thinking levels to provider/model-specific values; null marks a level unsupported. */
...
/**
 * Action implementations for Astro-Agent.* API methods.
