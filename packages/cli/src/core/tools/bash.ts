					...
					.then(({ exitCode }) => {
						...
						if (truncation.truncated) {
							...
							if (truncation.lastLinePartial) {
								outputText += `\n[truncated: partial line, full: ${tempFilePath}]`;
							} else if (truncation.truncatedBy === "lines") {
								outputText += `\n[truncated: ${truncation.outputLines}/${truncation.totalLines} lines, full: ${tempFilePath}]`;
							} else {
								outputText += `\n[truncated: ${truncation.outputLines}/${truncation.totalLines} lines (${formatSize(DEFAULT_MAX_BYTES)}), full: ${tempFilePath}]`;
							}
						}
...
export function createBashTool(cwd: string, options?: BashToolOptions): EngineTool<typeof bashSchema> {
