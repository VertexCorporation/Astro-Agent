...
export function migrateAuthToAuthJson(): string[] {
	...
	const authPath = join(engineDir, "auth.json");
	const oauthPath = join(engineDir, "oauth.json");
...
export function migrateSessionsFromEngineRoot(): void {
	...
	// Find all .jsonl files directly in engineDir (not in subdirectories)
	let files: string[];
	try {
		files = readdirSync(engineDir)
	...
	for (const file of files) {
		try {
			...
			const correctDir = join(engineDir, "sessions", safePath);
...
function migrateKeybindingsConfigFile(): void {
	const configPath = join(getEngineDir(), "keybindings.json");
...
function migrateToolsToBin(): void {
	const engineDir = getEngineDir();
	const toolsDir = join(engineDir, "tools");
...
function migrateExtensionSystem(cwd: string): string[] {
	...
	migrateCommandsToPrompts(engineDir, "Global");
	migrateCommandsToPrompts(projectDir, "Project");

	// Check for deprecated directories
	const warnings = [
		...checkDeprecatedExtensionDirs(engineDir, "Global"),
