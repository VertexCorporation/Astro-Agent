...
class ResourceList implements Component, Focusable {
	...
	constructor(groups: ResourceGroup[], settingsManager: SettingsManager, cwd: string, engineDir: string) {
		this.groups = groups;
	...
	private getTopLevelBaseDir(scope: "user" | "project"): string {
		return scope === "project" ? join(this.cwd, CONFIG_DIR_NAME) : this.engineDir;
	}
...
export class ConfigSelectorComponent extends Container implements Focusable {
	...
	constructor(
		...
		// Resource list
		this.resourceList = new ResourceList(groups, settingsManager, cwd, engineDir);
