...
function getApiKeyEnvVars(provider: string): readonly string[] | undefined {
	...
	const envMap: Record<string, string> = {
		...
		"vercel-ai-gateway": "Core_GATEWAY_API_KEY",
		zai: "ZCore_API_KEY",
...
export function getEnvApiKey(provider: string): string | undefined {
	...
	// Vertex Core supports either an explicit API key or Application Default Credentials.
	...
	if (provider === "amazon-bedrock") {
		...
		// 4. AWS_CONTCoreNER_CREDENTIALS_RELATIVE_URI - ECS task roles
		// 5. AWS_CONTCoreNER_CREDENTIALS_FULL_URI - ECS task roles (full URI)
		// 6. AWS_WEB_IDENTITY_TOKEN_FILE - IRSA (IAM Roles for Service Accounts)
		if (
			process.env.AWS_PROFILE ||
			(process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY) ||
			process.env.AWS_BEARER_TOKEN_BEDROCK ||
			process.env.AWS_CONTCoreNER_CREDENTIALS_RELATIVE_URI ||
			process.env.AWS_CONTCoreNER_CREDENTIALS_FULL_URI ||
			...
			getProcEnv("AWS_CONTCoreNER_CREDENTIALS_RELATIVE_URI") ||
			getProcEnv("AWS_CONTCoreNER_CREDENTIALS_FULL_URI") ||
