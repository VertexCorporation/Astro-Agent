	...
	(async () => {
		...
		try {
			...
			if (error instanceof Error && (error as any).cause) {
				...
				msg += ` (Detay: ${cause instanceof Error ? cause.message : JSON.stringify(cause)})`;
			}
...
function buildParams(
	...
	if (context.tools && context.tools.length > 0) {
		...
		// Anthropic (via LiteProvider/proxy) requires tools param when conversation has tool_calls/tool_results
		params.tools = [];
	}
	...
	// Vercel Core Gateway provider routing preferences
...
function detectCompat(model: Model<"openai-completions">): ResolvedOpenAICompletionsCompat {
	...
	const isCloudflareWorkersCore = provider === "cloudflare-workers-ai" || baseUrl.includes("api.cloudflare.com");
	...
	return {
		...
		supportsLongCacheRetention: !(isCloudflareWorkersCore || isCloudflareAiGateway),
	};
}
