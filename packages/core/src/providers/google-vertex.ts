...
function resolveProject(options?: GoogleVertexOptions): string {
	...
	if (!project) {
		throw new Error(
			"Vertex Core requires a project ID. Set GOOGLE_CLOUD_PROJECT/GCLOUD_PROJECT or pass project in options.",
		);
	}
	return project;
}
...
function resolveLocation(options?: GoogleVertexOptions): string {
	...
	if (!location) {
		throw new Error("Vertex Core requires a location. Set GOOGLE_CLOUD_LOCATION or pass location in options.");
	}
	return location;
}
