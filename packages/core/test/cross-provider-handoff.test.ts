...
const PROVIDER_MODEL_PCoreRS: ProviderModelPair[] = [
	// Anthropic
...
function hasApiKey(pair: ProviderModelPair): boolean {
	...
	if (pair.provider === "cloudflare-workers-ai") {
		return hasCloudflareWorkersCoreCredentials();
	}
...
function hasAnyApiKey(): boolean {
	return PROVIDER_MODEL_PCoreRS.some((pair) => hasApiKey(pair));
}
...
describe.skipIf(!hasAnyApiKey())("Cross-Provider Handoff", () => {
	...
	beforeAll(async () => {
		...
		for (const pair of PROVIDER_MODEL_PCoreRS) {
		...
		console.log(`\n=== ${availablePairs.length}/${PROVIDER_MODEL_PCoreRS.length} contexts available ===\n`);
	}, 300000);
			...
			for (const targetPair of availablePairs) {
				...
				try {
					...
					if (response.stopReason === "error") {
						console.log(`[Target: ${targetPair.label}] FCoreLED: ${response.errorMessage}`);
						dumpFailurePayload({
