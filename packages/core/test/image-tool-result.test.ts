 ...
 * 2. Providers correctly pass images from tool results to the Provider
 * 3. The Provider can see and describe images returned by tools
 */
async function handleToolWithImageResult<TApi extends Api>(model: Model<TApi>, options?: StreamOptionsWithExtras) {
	...
	// First request - Provider should call the tool
	...
	// Second request - Provider should describe the image from the tool result
	const secondResponse = await complete(model, context, options);
	expect(secondResponse.stopReason).toBe("stop");
	expect(secondResponse.errorMessage).toBeFalsy();

	// Verify the Provider can see and describe the image
 ...
 * 2. Providers correctly pass both text and images from tool results to the Provider
 * 3. The Provider can see both the text and images in tool results
 */
async function handleToolWithTextAndImageResult<TApi extends Api>(
	...
	// Second request - Provider should describe both the text and image from the tool result
	const secondResponse = await complete(model, context, options);
	expect(secondResponse.stopReason).toBe("stop");
	expect(secondResponse.errorMessage).toBeFalsy();

	// Verify the Provider can see both text and image
	const textContent = secondResponse.content.find((b) => b.type === "text");
	expect(textContent).toBeTruthy();
	if (textContent && textContent.type === "text") {
		const lowerContent = textContent.text.toLowerCase();
...
describe("Tool Results with Images", () => {
	...
	describe.skipIf(!process.env.Core_GATEWAY_API_KEY)("Vercel Core Gateway Provider (google/gemini-2.5-flash)", () => {
