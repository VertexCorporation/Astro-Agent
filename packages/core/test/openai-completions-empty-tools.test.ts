...
// Regression for https://github.com/badlogic/Astro-Agent/issues/<issue-number>
...
describe("openai-completions empty tools handling", () => {
	...
	it("preserves inline upstream Authorization for Cloudflare Core Gateway BYOK requests", async () => {
		process.env.CLOUDFLARE_ACCOUNT_ID = "account-id";
		process.env.CLOUDFLARE_GATEWAY_ID = "gateway-id";
	...
	it("sends session affinity headers for Workers Core through Cloudflare Core Gateway", async () => {
		process.env.CLOUDFLARE_ACCOUNT_ID = "account-id";
		process.env.CLOUDFLARE_GATEWAY_ID = "gateway-id";
	...
	it("still emits tools: [] for Anthropic/LiteProvider proxy when conversation has tool history", async () => {
		const { compat: _compat, ...baseModel } = getModel("openai", "gpt-4o-mini")!;
		const model = { ...baseModel, api: "openai-completions" } as const;
