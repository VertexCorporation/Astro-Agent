...
import type { EngineContext, EngineEvent, EngineLoopConfig, EngineMessage, EngineTool } from "../src/types.js";
...
function identityConverter(messages: EngineMessage[]): Message[] {
	return messages.filter((m) => m.role === "user" || m.role === "assistant" || m.role === "toolResult") as Message[];
}

describe("engineLoop with EngineMessage", () => {
	it("should emit events with EngineMessage types", async () => {
		...
		const userPrompt: EngineMessage = createUserMessage("Hello");

		const config: EngineLoopConfig = {
			model: createModel(),
		...
		const stream = engineLoop([userPrompt], context, config, undefined, streamFn);
		...
		expect(eventTypes).toContain("engine_start");
		expect(eventTypes).toContain("turn_start");
		expect(eventTypes).toContain("message_start");
		expect(eventTypes).toContain("message_end");
		expect(eventTypes).toContain("turn_end");
		expect(eventTypes).toContain("engine_end");
	});
	...
	it("should handle custom message types via convertToLlm", async () => {
		...
		const context: EngineContext = {
			systemPrompt: "You are helpful.",
			messages: [notification as unknown as EngineMessage], // Custom message in context
			tools: [],
		};
	...
	it("should apply transformContext before convertToLlm", async () => {
		...
		const userPrompt: EngineMessage = createUserMessage("new message");

		let transformedMessages: EngineMessage[] = [];
		let convertedMessages: Message[] = [];
	...
	it("should handle tool calls and results", async () => {
		...
		const tool: EngineTool<typeof toolSchema, { value: string }> = {
			name: "echo",
			label: "Echo",
		...
		const userPrompt: EngineMessage = createUserMessage("echo something");
	...
	it("should execute mutated beforeToolCall args without revalidation", async () => {
		...
		const tool: EngineTool<typeof toolSchema, { value: string | number }> = {
			name: "echo",
			label: "Echo",
			description: "Echo tool",
			parameters: toolSchema,
	...
	it("should prepare tool arguments for validation", async () => {
		...
		const tool: EngineTool<typeof toolSchema, { count: number }> = {
			name: "edit",
			label: "Edit",
		...
		const userPrompt: EngineMessage = createUserMessage("edit something");
		const config: EngineLoopConfig = {
			model: createModel(),
			convertToLlm: identityConverter,
		};
	...
	it("should emit tool_execution_end in completion order but persist tool results in source order", async () => {
		...
		const userPrompt: EngineMessage = createUserMessage("echo both");
		const config: EngineLoopConfig = {
			model: createModel(),
			convertToLlm: identityConverter,
		...
		let callIndex = 0;
		const stream = engineLoop([userPrompt], context, config, undefined, () => {
	...
	it("should inject queued messages after all tool calls complete", async () => {
		...
		const userPrompt: EngineMessage = createUserMessage("start");
		const queuedUserMessage: EngineMessage = createUserMessage("interrupt");
		...
		const events: EngineEvent[] = [];
		const stream = engineLoop([userPrompt], context, config, undefined, (_model, ctx, _options) => {
		...
		const toolEnds = events.filter(
			(e): e is Extract<EngineEvent, { type: "tool_execution_end" }> => e.type === "tool_execution_end",
		);
		...
		// Interrupt message should be in context when second Provider call is made
	...
	it("should force sequential execution when a tool has executionMode=sequential even with default parallel config", async () => {
		...
		const slowTool: EngineTool<typeof toolSchema, { value: string }> = {
			name: "slow",
			label: "Slow",
		...
		const userPrompt: EngineMessage = createUserMessage("run both");
	...
	it("should force sequential execution when one of multiple tools has executionMode=sequential", async () => {
		...
		const fastTool: EngineTool<typeof toolSchema, { value: string }> = {
			name: "fast",
			label: "Fast",
	...
	it("should stop after the current turn when shouldStopAfterTurn returns true", async () => {
		...
		let llmCalls = 0;
		const stream = engineLoop([createUserMessage("echo something")], context, config, undefined, () => {
			llmCalls++;
			const mockStream = new MockAssistantStream();
			queueMicrotask(() => {
				if (llmCalls === 1) {
	...
	it("should continue after parallel tool calls when not all tool results terminate", async () => {
		...
		let callIndex = 0;
		const stream = engineLoop([createUserMessage("echo both")], context, config, undefined, () => {
			const mockStream = new MockAssistantStream();
			queueMicrotask(() => {
				if (callIndex === 0) {
					const message = createAssistantMessage(
...
describe("engineLoopContinue with EngineMessage", () => {
	it("should throw when context has no messages", () => {
		...
		expect(() => engineLoopContinue(context, config)).toThrow("Cannot continue: no messages in context");
	});

	it("should continue from existing context without emitting user message events", async () => {
		const userMessage: EngineMessage = createUserMessage("Hello");
		...
		const events: EngineEvent[] = [];
		const stream = engineLoopContinue(context, config, undefined, streamFn);
		...
		// Should NOT have user message events (that's the key difference from engineLoop)
	...
	it("should allow custom message types as last message (caller responsibility)", async () => {
		...
		const context: EngineContext = {
			systemPrompt: "You are helpful.",
			messages: [customMessage as unknown as EngineMessage],
			tools: [],
		};
