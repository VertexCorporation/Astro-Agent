 ...
 */
const THCore_LAO_AM_REGEX = /[\u0e33\u0eb3]/;
const THCore_LAO_AM_GLOBAL_REGEX = /[\u0e33\u0eb3]/g;

export function normalizeTerminalOutput(str: string): string {
	if (!THCore_LAO_AM_REGEX.test(str)) return str;
	return str.replace(THCore_LAO_AM_GLOBAL_REGEX, (char) => (char === "\u0e33" ? "\u0e4d\u0e32" : "\u0ecd\u0eb2"));
}
...
/**
 * Premium metallic monochrome transitioning color wave generator.
 * Transitions smoothly from bright platinum white to soft silver and down to deep charcoal slate.
 */
export function getRainbowRGB(col: number, width: number, speedMultiplier = 1.0): { r: number; g: number; b: number } {
	const phase = (Date.now() / 1000) * 1.5 * speedMultiplier;
	const frequency = (2 * Math.PI) / Math.max(10, width);
	// Oscillate wave between -1 and 1
	const wave = Math.sin(phase - col * frequency);
	const t = (wave + 1) / 2; // 0 to 1

	// Map to soft dark charcoal (55, 60, 68) at t=0, and bright silver-white (245, 248, 255) at t=1
	const r = Math.round(55 + (245 - 55) * t);
	const g = Math.round(60 + (248 - 60) * t);
	const b = Math.round(68 + (255 - 68) * t);
	return { r, g, b };
}

/**
 * Colors a plain string character-by-character with a moving RGB gradient.
 */
export function colorizeRGB(str: string, speedMultiplier = 1.0, widthOverride?: number): string {
	let result = "";
	const len = str.length;
	const w = widthOverride || Math.max(80, len);
	for (let i = 0; i < len; i++) {
		const c = str[i];
		const { r, g, b } = getRainbowRGB(i, w, speedMultiplier);
		result += `\x1b[38;2;${r};${g};${b}m${c}`;
	}
	result += "\x1b[39m";
	return result;
}
...
export function sliceWithWidth(
	...
	// Ultimate Fix: Append reset code to prevent style bleeding from sliced lines
