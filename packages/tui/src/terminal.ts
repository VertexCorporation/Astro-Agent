...
export class ProcessTerminal implements Terminal {
	...
	private enableWindowsVTInput(): void {
		...
		try {
			...
			const modeIn = new Uint32Array(1);
			GetConsoleMode(hIn, modeIn);
			SetConsoleMode(hIn, modeIn[0]! | ENABLE_VIRTUAL_TERMINAL_INPUT);

			const hOut = GetStdHandle(STD_OUTPUT_HANDLE);
			const modeOut = new Uint32Array(1);
			GetConsoleMode(hOut, modeOut);
			SetConsoleMode(hOut, modeOut[0]! | ENABLE_VIRTUAL_TERMINAL_PROCESSING);
		} catch {
