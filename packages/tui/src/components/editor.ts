...
import { getSegmenter, isPunctuationChar, isWhitespaceChar, normalizeTerminalOutput, visibleWidth } from "../utils.js";
...
export class Editor implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		const layoutWidth = Math.max(1, contentWidth - 1 - 2 * this.paddingX);
		...
		if (textIsEmpty) {
			...
			const rightPadLen = Math.max(0, contentWidth - lineLen - this.paddingX);
			const rightPad = " ".repeat(rightPadLen);
			result.push(`${sideBorder}${padX}${lineText}${rightPad}${rightBorder}`);
		} else {
			const padX = " ".repeat(this.paddingX);
			for (const layoutLine of visibleLines) {
				let displayText = normalizeTerminalOutput(layoutLine.text);
				...
				const rightPadLen = Math.max(0, contentWidth - lineVisibleWidth - this.paddingX);
				const rightPad = " ".repeat(rightPadLen);
				result.push(`${sideBorder}${padX}${displayText}${rightPad}${rightBorder}`);
			}
		}
